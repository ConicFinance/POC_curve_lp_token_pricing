...
def calc_y_from_x_crv(x: Decimal, A: Decimal, D: Decimal, n: int = 2):
    ...
    for _ in range(255):
        y_prev = y
        y = (y**2 + c) / (2 * y + b - D)
        if abs(y - y_prev) < Decimal("0.01"):
            return y
    return y


def calc_x_from_D(D: Decimal, A: Decimal, price: Decimal, n: int = 2):
    x_cur = D
    x_prev = Decimal(0)
    for _ in range(255):
        x_cur = next_iter(D, A, x_cur, price, n)
        if abs(x_cur - x_prev) < Decimal("0.01"):
            return x_cur
        x_prev = x_cur
...
def get_v1_lp_token_price(
    ...
    amount_asset_a = calc_x_from_D(D, A_precise, price_a)
